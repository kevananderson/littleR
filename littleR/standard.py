...
class Standard:
    ...
    def add_requirement(self, requirement):
        ...
        if requirement.index in self.req_dict:
            self.validator.file_note(f'Duplicate index: {requirement.index}.', requirement.file, problem=True)
            return
        ...
        for req in self.req_dict.values():
            ...
            for i, parent_idx in enumerate(req.parent_idx):
                ...
                if parent_idx not in self.req_dict:
                    self.validator.index_note(f'Parent index not found: {parent_idx}.', req.file, req.index, problem=True)
            ...
            for i, child_idx in enumerate(req.child_idx):
                ...
                if child_idx not in self.req_dict:
                    self.validator.index_note(f'Child index not found: {child_idx}.', req.file, req.index, problem=True)
            ...
            for i, related_idx in enumerate(req.related_idx):
                if related_idx in self.new_requirements:
                    related_idx = req.related_idx[i] = self.new_requirements[related_idx]

                if related_idx not in self.req_dict:
                    self.validator.index_note(f'Related index not found: {related_idx}.', req.file, req.index, problem=True)
